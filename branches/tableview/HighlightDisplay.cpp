...
/*Deprecated function waiting for further work.  We need to be able to embed QComboBox in a QStandardItemModel*/
void HighlightDisplay::addTableView()
{
	QStringList numbers;
 	numbers << "One" << "Two" << "Three" << "Four" << "Five";

	QStandardItemModel *model = new QStandardItemModel();//numbers.size(), 1);//QAbstractItemModel*
	for(int i = 0; i < numbers.size(); ++i)
	{
		QStandardItem *item = new QStandardItem(numbers[i]);
		model->setItem(i, 0, item);
		//ColorListEditor colorBox = ColorListEditor();
		//model->setItem(i, 1, new QStandardItem(colorBox));
		//model->appendRow(numbers[i]);
	}
	QTableView* tableView = new QTableView(settingsBox);
	//QListView *view = new QListView(settingsBox);
	tableView->setModel(model);
	formLayout->addWidget(tableView);
	tableView->show();
}
