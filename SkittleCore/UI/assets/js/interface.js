...
function mouseWheelDials(e) {
    ...
    if (delta > 0.05 || delta < -0.05) {
        switch ($(this).children('span').attr('id')) {
            case "widthDisplay":
                state.width(by(-delta * 10));
                return;
            case "scaleDisplay":
                state.scale(by(-delta * 5));
                return;
            default:
                state.start(by(-delta * 10 * state.scale()));
                return;
        }
    }
}
...
$(function () {
    ...
    $('.highlighterSettings').on('change', '#similarityPercent', function () {
        if ($(this).simpleSlider) {
            $('#similarityPercent').simpleSlider("setValue", $(this).val());
        }
    })
...
var loadHighlighterSettings = function (hState) {
    ...
    try {
        $('#similarityPercent').simpleSlider("setValue", hState.similarityPercent);
    }
    catch (e) {
...
StateObject.prototype.start = getterSetter({defaultVal: 1, filter: /(\d+)/, min: 1, max: function () {
    return fileLength - (skixelsOnScreen - (25 + 37) * state.width()) * state.scale() / 2 + 1
}})
StateObject.prototype.scale = getterSetter({defaultVal: 1, filter: /(\d+)/, min: 1, max: 5000})
StateObject.prototype.end = function (a) {
    return state.start(a.match(/(\d+)/)[0] - ( skixelsOnScreen - (25 + 37) * state.width() - 1 ) * state.scale())
}
StateObject.prototype.bpPerLine = function () {
    return this.width() * this.scale()
}
StateObject.prototype.goToEnd = function () {
    return this.start(fileLength);
}
...
var lines = function (a) {
    return Math.round(a) * state.bpPerLine();
}
var by = function (d) {
    return function (a) {
        return a + d;
